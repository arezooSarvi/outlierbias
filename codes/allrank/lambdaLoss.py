...
def lambdaLoss(y_pred, y_true, eps=DEFAULT_EPS, padded_value_indicator=PADDED_Y_VALUE, weighing_scheme=None, k=None, sigma=1., mu=10.,
    ...
    if reduction == "sum":
        loss = -torch.sum(losses[padded_pairs_mask.type(torch.uint8) & ndcg_at_k_mask.type(torch.uint8)])
    elif reduction == "mean":
        loss = -torch.mean(losses[padded_pairs_mask.type(torch.uint8) & ndcg_at_k_mask.type(torch.uint8)])
    else:
