...
from allrank.transformer import make_transformer
from allrank.python_utils import instantiate_class
...
class FCModel(nn.Module):
    ...
    def __init__(self, sizes, input_norm, activation, dropout, n_features):
        ...
        layers = [nn.Linear(n_features, sizes[0])]
        for i in range(1, len(sizes)):
            if dropout is not None:
                layers.append(nn.Dropout(p=dropout, inplace=True))
            layers.append(self.activation)
            layers.append(nn.Linear(sizes[i-1], sizes[i], bias=False))
...
class OutputLayer(nn.Module):
    ...
    def __init__(self, d_model, d_output, output_activation=None):
        ...
        layers = [nn.Linear(d_model, d_output)]
        layers.append(self.activation)
        self.layers = nn.Sequential(*layers)
    ...
    def forward(self, x):
        ...
        return self.layers(x).squeeze(dim=2)
...
def make_model(fc_model, transformer, post_model, n_features):
    ...
    if transformer:
        transformer = make_transformer(n_features=d_model, **transformer)  # type: ignore
